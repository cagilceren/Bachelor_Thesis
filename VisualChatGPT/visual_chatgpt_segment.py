...
class Text2Box:
    ...
    def plot_boxes_to_image(self, image_pil, tgt):
        ...
        return image_pil, mask, number_of_boxes
    ...
    def inference(self, inputs):
        ...
        number_of_boxes = self.plot_boxes_to_image(image_pil, pred_dict)[2]
        ...
        print(
            ...
            f"Output Image: {updated_image_path}")
        return updated_image_path, number_of_boxes
...
class ConversationBot:
    ...
    def run_image(self, img, lang):
       ...
       # img = Image.open(image.name)
        ...
        print(img.save(image_filename, "PNG"))
        print(f"Resize image form {width}x{height} to {width_new}x{height_new}")
        description = self.models['ImageCaptioning'].inference(image_filename)
        answer = str(self.models["Text2Box"].inference(image_filename)[1])
        if lang == 'Chinese':
...
if __name__ == '__main__':
    ...
    if not os.path.exists(path):
        os.mkdir(path)
        print(f"a new {path} directory is created!")
    for(root, dirs, files) in os.walk("."):
        if (root == "./data"):
            for dir in dirs:
                information = []
                for image in os.listdir(f"./data/{dir}"):
                    if image.endswith(".png"):
                        Image.open(f'./data/{dir}/{image}')
                        description, answer = bot.run_image(Image.open(f'./data/{dir}/{image}'), 'English')
                        information.append([image, answer + ';'])
                filename = f"./{path}/segmentation_predictions_{dir}.csv"
